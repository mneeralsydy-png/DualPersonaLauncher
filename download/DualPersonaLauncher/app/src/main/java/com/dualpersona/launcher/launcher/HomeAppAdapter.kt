...
/**
 * RecyclerView adapter for home screen apps.
 * Displays a grid of app icons with labels for the current environment.
 */
class HomeAppAdapter(
    private val onAppClick: (AppInfoEntity) -> Unit,
    private val onAppLongClick: (AppInfoEntity) -> Unit
) : ListAdapter<AppInfoEntity, HomeAppAdapter.AppViewHolder>(AppDiffCallback) {
